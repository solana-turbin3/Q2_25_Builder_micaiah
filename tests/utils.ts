...
import { createMint, getOrCreateAssociatedTokenAccount, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getAccount, TOKEN_2022_PROGRAM_ID, getMint, createSetAuthorityInstruction, AuthorityType } from "@solana/spl-token";
...
export async function initializeProtocol(
    ...
    if (configInfo === null) {
        ...
        if (cnMintAccount.mintAuthority.toBase58() !== configPda.toBase58()) {
            ...
            const sig = await provider.connection.sendTransaction(tx, [initializer], {skipPreflight: true});
        ...
        if (collectionMintAccount.mintAuthority.toBase58() !== configPda.toBase58()) {
            const tx = new Transaction().add(
                createSetAuthorityInstruction(
                    collectionMintPk,
                    initializer.publicKey,
        ...
        const tx = await program.methods.initialize(optionDurationSeconds)
            .accountsStrict({
...
export async function sendAndConfirmTransaction(
    ...
    options?: anchor.web3.ConfirmOptions) {
        ...
        const sig = await provider.connection.sendRawTransaction(await transaction.serialize(), options);
        await confirmTransaction(provider, sig);
    }
...
*/
export async function confirmTransaction(provider: anchor.AnchorProvider, signature: string) {
    let confirmed = false;
    while (confirmed == false) {
        let status = await provider.connection.getSignatureStatus(signature);
        if (status.value.err) {
            throw new Error(`Transaction failed: ${signature}, error: ${JSON.stringify(status.value.err)}`);
        }

...
import { getOrCreateAssociatedTokenAccount, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getAccount, TOKEN_2022_PROGRAM_ID } from "@solana/spl-token"; // import token_2022_program_id
import {
    CN_MINT_ADDRESS,
    PT_MINT_ADDRESS,
    ...
    it("fails deposit when protocol is globally locked", async () => {
        ...
        try {
            await program.methods.deposit(depositAmount).accounts({
                 ...
                 systemProgram: SystemProgram.programId,
                 rent: anchor.web3.SYSVAR_RENT_PUBKEY,
            }).signers([depositor]).rpc({ commitment: "confirmed" });

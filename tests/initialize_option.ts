...
describe("initialize_option instruction", () => {
  ...
  it("initializes an option NFT, metadata, master edition, and OptionData PDA", async () => {
    ...
    assert.strictEqual(
      ...
      "1",
      "user Option ATA should hold 1 NFT"
    );
    assert.ok(
      userOptionAccount.mint.equals(optionMint.publicKey),
      "option ATA mint mismatch"
    );
    ...
    assert.ok(
      optionDataAccount.mint.equals(optionMint.publicKey),
      "OptionData mint mismatch"
    );
    assert.ok(
      optionDataAccount.owner.equals(user.publicKey),
      "OptionData owner mismatch"
    );
    assert.strictEqual(
      optionDataAccount.amount.toString(),
      depositAmount.toString(),
      "OptionData amount mismatch"
    );
    assert.ok(
      optionDataAccount.expiration.toNumber() > 0,
      "OptionData expiration should be set"
    );
    ...
    expect(optionDataAccount.expiration.toNumber()).to.be.closeTo(
      expectedExpiration,
      60
    ); // allow 60s tolerance

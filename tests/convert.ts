...
import { getOrCreateAssociatedTokenAccount, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getAccount, TOKEN_2022_PROGRAM_ID } from "@solana/spl-token";
import {
    CN_MINT_ADDRESS,
    PT_MINT_ADDRESS,
...
describe("convert instruction (with hardcoded mints)", () => {
    ...
    before(async () => {
        ...
        protocolPtAta = await anchor.utils.token.associatedAddress({ mint: ptMint, owner: configPda }); // derive protocol PT ATA

        console.log("--- Setting up for Convert Test ---");

        // 1. perform deposit
        console.log("performing deposit for converter...");
        converterCnAta = await anchor.utils.token.associatedAddress({ mint: cnMint, owner: converter.publicKey });
        ...
        converterOptionAta = await anchor.utils.token.associatedAddress({ mint: optionMintKp.publicKey, owner: converter.publicKey });
        ...
        await program.methods
            ...
            .rpc({ commitment: "confirmed" });
        console.log(`option initialized. Mint: ${optionMintKp.publicKey.toBase58()}, PDA: ${optionDataPda.toBase58()}`);
    ...
    it("allows conversion when protocol is unlocked & verifies state changes", async () => {
        ...
        const initialCnAccount = await getAccount(provider.connection, converterCnAta);
        const initialCnBalance = initialCnAccount.amount;
        const initialProtocolPtAccount = await getAccount(provider.connection, protocolPtAta);
        ...
        // 2. CN token burn
        const finalCnAccount = await getAccount(provider.connection, converterCnAta);
        ...
        const optionDataInfo = await provider.connection.getAccountInfo(optionDataPda);
        assert.isNull(optionDataInfo, "OptionData account should be closed");

        // 4. NFT burn (check token account balance is zero or closed)
        try {
            const finalOptionAccount = await getAccount(provider.connection, converterOptionAta);
        ...
        const finalProtocolPtAccount = await getAccount(provider.connection, protocolPtAta);
    ...
    it("fails conversion when protocol is globally locked", async () => {
        ...
        const localCnAta = await anchor.utils.token.associatedAddress({ mint: cnMint, owner: localConverter.publicKey });
        await program.methods.deposit(depositAmount).accounts({ depositor: localConverter.publicKey, depositor_sol_account: localConverter.publicKey, config: configPda, treasury: treasuryPda, cn_mint: cnMint, pt_mint: ptMint, protocol_pt_ata: protocolPtAta, token_program: TOKEN_2022_PROGRAM_ID, associated_token_program: ASSOCIATED_TOKEN_PROGRAM_ID, system_program: SystemProgram.programId, rent: anchor.web3.SYSVAR_RENT_PUBKEY }).signers([localConverter]).rpc();
        const localOptionMintKp = Keypair.generate();
        const localOptionAta = await anchor.utils.token.associatedAddress({ mint: localOptionMintKp.publicKey, owner: localConverter.publicKey });
        const localMetadataPda = findMetadataPda(localOptionMintKp.publicKey);
        const localMasterEditionPda = findMasterEditionPda(localOptionMintKp.publicKey);
        const [localOptionDataPda] = PublicKey.findProgramAddressSync([Buffer.from("option_data"), localOptionMintKp.publicKey.toBuffer()], program.programId);
        await program.methods.initializeOption(depositAmount).accounts({ payer: localConverter.publicKey, config: configPda, option_mint: localOptionMintKp.publicKey, user_option_ata: localOptionAta, metadata_account: localMetadataPda, master_edition_account: localMasterEditionPda, option_data: localOptionDataPda, token_program: TOKEN_PROGRAM_ID, associated_token_program: ASSOCIATED_TOKEN_PROGRAM_ID, system_program: SystemProgram.programId, rent: anchor.web3.SYSVAR_RENT_PUBKEY, token_metadata_program: TOKEN_METADATA_PROGRAM_ID }).signers([localConverter, localOptionMintKp]).rpc();
        ...
        const localPtAta = await anchor.utils.token.associatedAddress({ mint: ptMint, owner: localConverter.publicKey });
        ...
        try {
            await program.methods.convert().accounts({
                ...
                sysvar_instructions: anchor.web3.SYSVAR_INSTRUCTIONS_PUBKEY,
                rent: anchor.web3.SYSVAR_RENT_PUBKEY,
            }).signers([localConverter]).rpc({ commitment: "confirmed" });
